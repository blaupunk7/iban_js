// banktransfer_iban.html & banktransfer_iban.js 1.5 - Create or check International Bank Account Numbers
...
// Interface functions for banktransfer_iban.html:
// CreateIBAN
// CheckIBAN
...
// Required tags in banktransfer_iban.html:
// Form             checkout_payment
//   Selection bar  checkout_payment.country
//   Input field    checkout_payment.banktransfer_blz
//   Input field    checkout_payment.banktransfer_number
//   Input field    checkout_payment.banktransfer_iban
//   Output field   checkout_payment.alt_iban
// Image            bban_img
// Image            iban_img

// Used images:
// okay.png         check-mark in checkout_payment
// error.png        question-mark in checkout_payment
...
// JavaScript Object for country specific banktransfer_iban data.
function Country(name, code, bank_form, acc_form)
{
  ...
  //   bank_form - Format of banktransfer_blz/branch code part (e.g. "0 4a 0 ")
  //   acc_form  - Format of banktransfer_number number part (e.g. "0  11  2n")

  this.name      = name;
  this.code      = code;
  this.banktransfer_blz      = Country_decode_format(bank_form);
  this.acc       = Country_decode_format(acc_form);
  this.bank_lng  = Country_calc_length(this.banktransfer_blz);
...
function ChecksumIBAN(banktransfer_iban)
{
  var code     = banktransfer_iban.substring(0, 2);
  var checksum = banktransfer_iban.substring(2, 4);
  var bban     = banktransfer_iban.substring(4);
...
// Fill the banktransfer_number number part of IBAN with leading zeros.
function FillAccount(country, banktransfer_number)
{
  return fill0(banktransfer_number, country.acc_lng);
}
...
// Check if length of the banktransfer_blz/branch code part of IBAN is invalid.
function InvalidBankLength(country, banktransfer_blz)
{
  return (banktransfer_blz.length != country.bank_lng);
}

// Check if syntax of the banktransfer_blz/branch code part of IBAN is invalid.
function InvalidBank(country, banktransfer_blz)
{
  return (InvalidBankLength(country, banktransfer_blz) ||
          InvalidPart(country.banktransfer_blz, banktransfer_blz));
}

// Check if length of the banktransfer_number number part of IBAN is invalid.
function InvalidAccountLength(country, banktransfer_number)
{
  return (banktransfer_number.length < 1 || banktransfer_number.length > country.acc_lng);
}

// Check if syntax of the banktransfer_number number part of IBAN is invalid.
function InvalidAccount(country, banktransfer_number)
{
  return (InvalidAccountLength(country, banktransfer_number) ||
          InvalidPart(country.acc, FillAccount(country, banktransfer_number)));
}

// Check if length of IBAN is invalid.
function InvalidIBANlength(country, banktransfer_iban)
{
  return (banktransfer_iban.length != country.total_lng);
}

// Convert banktransfer_iban from intern value to string format (IBAN XXXX XXXX ...).
...
// Convert banktransfer_iban from string format to intern value.
...
function CalcIBAN(country, banktransfer_blz, banktransfer_number)
{
  var fill_acc = FillAccount(country, banktransfer_number);
  var checksum = ChecksumIBAN(country.code + "00" + banktransfer_blz + fill_acc);
  return country.code + checksum + banktransfer_blz + fill_acc;
}

function CalcAltIBAN(country, banktransfer_blz, banktransfer_number)
{
  var fill_acc = FillAccount(country, banktransfer_number);
  var checksum = ChecksumIBAN(country.code + "00" + banktransfer_blz + fill_acc);
...
function IBANokay(banktransfer_iban)
{
  return ChecksumIBAN(banktransfer_iban) == "97";
}
...
function CreateIBAN()
{
  ...
  var banktransfer_blz = intern(form.banktransfer_blz.value);
  var banktransfer_number = intern(form.banktransfer_number.value);
  ...
  else if (InvalidBankLength(country, banktransfer_blz))
  {
    err = _("Bank/Branch Code length ") + banktransfer_blz.length +
          _(" is not correct for ") + country.name +
          " (" + country.bank_lng + ")";
    err_focus = form.banktransfer_blz;
  }
  else if (InvalidBank(country, banktransfer_blz))
  {
    err = _("Bank/Branch Code ") + banktransfer_blz + _(" is not correct for ") +
          country.name;
    err_focus = form.banktransfer_blz;
  }
  else if (InvalidAccountLength(country, banktransfer_number))
  {
    err = _("Account Number length ") + banktransfer_number.length +
          _(" is not correct for ") + country.name +
          " (" + country.acc_lng + ")";
    err_focus = form.banktransfer_number;
  }
  else if (InvalidAccount(country, banktransfer_number))
  {
    err = _("Account Number ") + banktransfer_number + _(" is not correct for ") +
          country.name;
    err_focus = form.banktransfer_number;
  }
  ...
  //  document.bban_img.src = blank_img.src;
    ...
    form.banktransfer_blz.value = banktransfer_blz;
    form.banktransfer_number.value = FillAccount(country, banktransfer_number);
    form.banktransfer_iban.value = extern(CalcIBAN(country, banktransfer_blz, banktransfer_number));
	//regex
	var x = document.getElementById("banktransfer_iban").value;
	x = x.replace(/\s+/g,"");
	document.getElementById("banktransfer_iban").value = x;
	//update iban field
	document.getElementById("3").value = x;

    // Calculate alternative IBAN, write warning if not the same
    form.alt_iban.value = extern(CalcAltIBAN(country, banktransfer_blz, banktransfer_number));
    if (form.alt_iban.value != form.banktransfer_iban.value)
      form.alt_iban.value += " (*)";
    else
      form.alt_iban.value = "";
...
{
  var form = document.checkout_payment;
  var banktransfer_iban = intern(form.banktransfer_iban.value);
  ...
  if (country == null)
    err = _("Unknown Country Code: ") + code;
  else if (InvalidIBANlength(country, banktransfer_iban))
    err = _("IBAN length ") + banktransfer_iban.length + _(" is not correct for ") +
          country.name + " (" + country.total_lng + ")";
  else
  {
    var bank_lng = country.bank_lng;
    var banktransfer_blz     = bban.substring(0, bank_lng);
    var banktransfer_number  = bban.substring(bank_lng);

    if (InvalidBank(country, banktransfer_blz))
      err = _("Bank/Branch Code ") + banktransfer_blz + _(" is not correct for ") +
            country.name;
    else if (InvalidAccount(country, banktransfer_number))
      err = _("Account Number ") + banktransfer_number + _(" is not correct for ") +
            country.name;
    else if (!IBANokay(banktransfer_iban))
  ...
  if (err)
  {
   ...
   // document.bban_img.src = blank_img.src;
    ...
    form.banktransfer_blz.value = "";
    form.banktransfer_number.value = "";
    form.alt_iban.value = "";
    form.banktransfer_iban.focus();
...
// Write a table with the country specific banktransfer_iban format.
function WriteCountryFormatTable()
{
  document.write('<table bgcolor="#99FFCC" width="100%" border="4">' +
                 ...
                 ' </tr>' +
                 ' <tr>' +
                 '  <th>' + _("check1") + '</th><th>' + _("banktransfer_blz") + '</th>' +
  ...
  for (var i = 0; i < tab.length; ++i)
  {
    ...
    for (var f = 0; f < country.banktransfer_blz.length; ++f)
    {
      var lng = country.banktransfer_blz[f][0], typ = country.banktransfer_blz[f][1];
      if (lng > 0)
...
// Write a table with banktransfer_iban test data.
function WriteTestTable(data)
{
  ...
  {
    var code     = data[i][0];
    var banktransfer_blz     = data[i][1];
    var banktransfer_number  = data[i][2];
    var checksum = data[i][3];
    var country  = CountryData(code);
    var banktransfer_iban = "", err = null;
    ...
    {
      banktransfer_iban = CalcIBAN(country, banktransfer_blz, banktransfer_number);

      if (banktransfer_iban.substring(2, 4) != checksum)
      {
        var alt_iban = CalcAltIBAN(country, banktransfer_blz, banktransfer_number);
        if (alt_iban.substring(2, 4) == checksum)
          banktransfer_iban = alt_iban;
      }

      if (banktransfer_iban.substring(0, 2) != code)
        err = _("Country code changed");
      else if (InvalidIBANlength(country, banktransfer_iban))
        err = _("Incorrect IBAN length: ") + banktransfer_iban.length +
              " (" + country.total_lng + ")";
      else
      {
        var bban = banktransfer_iban.substring(4);
        ...
        if (bban.substring(0, bank_lng) != banktransfer_blz)
          err = _("Bank/Branch Code changed");
        else if (bban.substring(bank_lng) != FillAccount(country, banktransfer_number))
          err = _("Account Number changed");
        else if (!IBANokay(banktransfer_iban))
          err = _("Incorrect checksum");
        else if (banktransfer_iban.substring(2, 4) != checksum)
          err = _("Checksum changed");
      }
    }
    document.write(' <tr>' +
                   '  <td align="center">' + code + '</td>' +
                   '  <td align="center">' + banktransfer_blz + '</td>' +
                   '  <td align="center">' + banktransfer_number + '</td>');
    if (err)
      ...
      document.write('  <td align="center">' +
                     ...
                     '        border="0" alt="<==>" />' +
                     '  </td>' +
                     '  <td align="center">' + extern(banktransfer_iban) + '</td>' +
...
function debug_iban_data()
{
  ...
  for (var i = 0; i < iban_data.length; ++i)
  {
    ...
    for (var f = 0; f < country.banktransfer_blz.length; ++f)
      s += country.banktransfer_blz[f][0] + country.banktransfer_blz[f][1];
    s += " = " + country.bank_lng + " / ";
    for (var f = 0; f < country.acc.length; ++f)
